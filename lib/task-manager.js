...
TaskManager.prototype.start = function(){
  ...
  self.cronTasks.push(nodeSchedule.scheduleJob(timers.follow.cron, function(){
    ...
    var nextFollow = self.state.db('shouldFollows').chain().sortBy('addedOn').reverse().find(function(v){
      if(_.includes(QUEUED_ITEMS, v.id_str)) return false;
      var doesFollow = self.state.db('followedUsers').find({id_str: v.id_str}) || self.state.db('friends').find({id_str: v.id_str});
      // Make sure we haven't already followed this user yet to prevent race condition.
      if(doesFollow){
        self.state.db('shouldFollows').remove({id_str: v.id_str});
        self.state.db.save();
        return false;
      }
    ...
    if(nextFollow){
      ...
      logger.info('Running follow task for user', nextFollow.id_str);
  ...
  self.cronTasks.push(nodeSchedule.scheduleJob(timers.unfollow.cron, function(){
    ...
    if(nextUnfollow){
      logger.info('Running un-follow task for user', nextUnfollow.id_str);
...
TaskManager.prototype.followUser = function(user){
  ...
  var finished = function(){
    logger.info('Follow task complete for user', user.id_str);
...
TaskManager.prototype.unfollowUser = function(user){
  ...
  var finished = function(){
    logger.info('Unfollow task complete for user', user.id_str);
